...
episodes = 750       # Total number of training episodes
...
for e in range(episodes):
    ...
    while not done:
        ...
        next_state, reward, terminated, truncated, info = env.step(action)            # Execute action in environment
        done = terminated or truncated                          #Check if an episode is done
...
# --- MATPLOTLIB PLOT ---
window_size = 100  # Window size for calculating the moving average
...
plt.title('Training Q-Learning - Taxi-v4')
plt.xlabel('Episode')
plt.ylabel('Total Reward')
plt.axhline(y=5, color='r', linestyle='--', alpha=0.5)  # Reference benchmark line
plt.legend()
